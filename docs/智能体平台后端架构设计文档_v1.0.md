...
| **Agent Runtime** | 集成 AgentScope 核心框架，支持 ChatAgent 对话执行 | - [x] 运行时接口封装<br>- [x] 同步/流式执行接口<br>- [ ] AgentScope 真实集成（当前为 mock）<br>- [ ] ChatAgent 实际调用<br>- [ ] 真实 LLM 对接 |
...
| **里程碑** | 内部 Demo 演示，核心链路可用 | ⚠️ 核心链路已打通（注册→登录→创建智能体→对话），但 Agent 执行为 mock，RAG 未实现 |
