...
| 交付物 | 说明 |
|--------|------|
| **项目脚手架** | FastAPI 项目结构、Docker Compose 环境（PostgreSQL + Redis + RabbitMQ + MinIO）、Alembic 迁移、CI/CD 基础流水线 |
| **认证与多租户** | 注册/登录/JWT/Refresh Token、Casbin RBAC、多租户数据隔离 |
| **智能体管理** | Agent CRUD + 版本化发布 + 配置管理 |
| **Agent Runtime** | 集成 AgentScope 核心框架，支持 ChatAgent 对话执行 |
| **会话管理** | 会话创建/消息收发/历史查询、SSE 流式输出 |
| **RAG 基础** | 知识库 CRUD、文档上传解析、向量化索引、基础语义检索 |
| **里程碑** | 内部 Demo 演示，核心链路可用 |
